...
VALUES("Ivanov Ivanov", "9207186371", "София-Сердика","10","0888892950");
...
select sportgroups.location, sportgroups.houroftraining, sportgroups.dayofweek, sports.name
from sportgroups
JOIN sports on sports.id = sportgroups.sport_id
where sports.name = 'Volleyball';

select sports.name, students.name
from sports
...
select students.name, sum(taxespayments.paymentAmount) as payment
from students
join taxespayments on students.id = taxespayments.student_id
GROUP BY students.name
ORDER BY name asc;

create view students_order_football as
select students.name, students.phone, students.class
from students
join student_sport on students.id = student_sport.student_id
join sportgroups on sportgroups.id = student_sport.sportgroup_id
join sports on sports.id = sportgroups.sport_id
...
select * FROM students_order_football;

create view coaches_volleyball_names as
...
select * from  coaches_volleyball;
