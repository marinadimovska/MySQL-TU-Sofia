...
SELECT DISTINCT students.name, students.phone, students.class
from students 
join student_sport on students.id = student_sport.student_id
join sportgroups on sportgroups.sport_id = student_sport.sportGroup_id
...
select sum(taxespayments.paymentAmount)
from taxespayments
...
GROUP BY sportgroups.sport_id;

SELECT DISTINCT CONCAT_WS(' and ', s1.name, s2.name) AS 'Student Pair'
FROM students s1
JOIN student_sport ss1 ON ss1.student_id = s1.id
JOIN sportgroups sg1 ON sg1.id = ss1.sportGroup_id
JOIN sports sp1 ON sp1.id = sg1.sport_id 
JOIN student_sport ss2 ON ss2.sportGroup_id = ss1.sportGroup_id AND ss2.student_id != ss1.student_id
JOIN students s2 ON s2.id = ss2.student_id
WHERE s1.id < s2.id AND sp1.name = 'Football';

create view Training_in_8oclock as
SELECT students.name as students, students.class, sportgroups.location, coaches.name
from students
join student_sport on students.id = student_sport.student_id
join sportgroups on sportgroups.id = student_sport.sportGroup_id
join coaches on coaches.id = sportgroups.coach_id
...
SELECT * from Training_in_8oclock;
