...
insert into `school_sport_clubs`.students (name, egn, address, class, phone)
values("Ivan Ivanov Ivanov", "9207186373", "Sofia-Serdika", "10", "0888892950");
...
delete from `school_sport_clubs`.students 
where id =13;

select students.name, sp.name from students Join student_sport ss On students.id = ss.student_id
...
ON students.id IN(
    SELECT student_id
    from student_sport
    WHERE student_sport.sportGroup_id = sport_id
    )
...
ON students.id IN(
    SELECT student_id
    FROM student_sport
    WHERE student_sport.sportGroup_id = sports.id
    )
